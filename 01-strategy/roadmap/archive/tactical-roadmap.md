# Tactical Roadmap (12-Month)

**Last Updated:** 2025-12-09  
**Roadmap Period:** FY26 (July 2025 - June 2026)  
**Type:** Strategic Roadmap (6-12 Month Horizon) + Portfolio Roadmap (3-6 Month Horizon)  
**Framework Layers:** Strategic Roadmap + Portfolio Roadmap  
**Update Frequency:** Strategic elements quarterly; Portfolio elements monthly/fortnightly

## Overview

This tactical roadmap provides the detailed 12-month execution plan derived from our [Strategic Roadmap (2-Year Vision)](strategic-roadmap.md). It includes specific quarterly milestones, deliverables, and tactical initiatives that will drive us toward our strategic goals.

**Note:** This roadmap combines elements of both the **Strategic Roadmap** (6-12 months) and **Portfolio Roadmap** (3-6 months) layers from Forwood's [4-layer roadmapping framework](roadmapping-framework.md). As we evolve our roadmap structure, we may separate these into distinct roadmap documents.

## Strategic Alignment

This tactical roadmap supports the following strategic themes:
- **AI & Predictive Safety** - Launch AI features from innovation stream (AI Safety Coach, AI risk identification & control verification, predictive insights, auto translation & coaching)
- **Modern User Experience** - Deliver design system and achieve usability targets (redesigned desktop & mobile app, clean UI/UX, offline capability, role dashboards, quick actions, streamlined workflows)
- **Integration & Interoperability** - Enable customer migration and platform integration (CRM connects with Risk, Safety, Compliance; APIs for HR, asset management, construction planning)
- **Data, Benchmarking & Insights** - Launch analytics and maturity model (Maturity Model, real-time analytics & exec dashboards, benchmarking, C-suite reporting)
- **Scalable Platform** - Complete Forwood One platform launch and migration (cloud-native, modular, configurable; faster innovation; extensible to AI/IoT/vision systems)

## Business Outcomes

What business outcomes are we driving toward this year?
- [Forwood One Platform Launch and New Customer Acquisition](oculus-platform-launch-new-deals.md)
- [Safety Plus Upgrade](safety-plus-upgrade.md)
- [CRM Upgrade Pathway](crm-upgrade-pathway.md)
- [Risk and Compliance Platform Integration](risk-compliance-integration.md)
- [Strategic Integration Plan](strategic-integration-plan.md)

## Quarterly Tactical Plan

### Q1 (July-Sep 2025)

**Focus:** Foundation and MVP Launch

**Key Deliverables:**
- Define and embed agentic product processes
- Launch Forwood One MVP
- Complete UAT and train wider team
- Establish product discovery and delivery cadence

**Milestones:**
- ✅ Forwood One MVP launched
- ✅ Team trained on new platform
- ✅ Product processes embedded

**Success Metrics:**
- MVP launch on schedule
- Team readiness for Q2 activities

---

### Q2 (Oct-Dec 2025)

**Focus:** Platform Enhancement and Sales Enablement

**Key Deliverables:**
- Onboard and embed Product Designer
- Launch Forwood One V1 (UAT and train wider team)
- Launch Forwood One V2 (UAT and ready for migration)
- Validate Risk+ iframe in Forwood One with UAT and possible migration
- Implement branding and new platform name

**Milestones:**
- ✅ Product Designer onboarded
- ✅ Forwood One V1 launched
- ✅ Forwood One V2 launched
- ✅ Sales have >15 deals in Forwood One pipeline (Dec)
- ✅ Sales actively conducting demonstrations (Dec)

**Success Metrics:**
- V2 platform ready for customer migration
- Sales pipeline targets met
- Risk+ integration validated

---

### Q3 (Jan-Mar 2026)

**Focus:** Customer Migration and Native App Launch

**Key Deliverables:**
- Begin migration of Safety+ clients to Forwood One
- Deeply integrate new design principles and systems
- Launch MVP Forwood One Native App
- Develop and Present State of the Product (Internal/External)

**Milestones:**
- ✅ First Safety+ client migrations completed
- ✅ First deals signed for Forwood One (Feb/Mar)
- ✅ Native app MVP launched
- ✅ State of the Product presented

**Success Metrics:**
- Successful customer migrations
- New customer deals closed
- Native app adoption begins

---

### Q4 (Apr-Jun 2026)

**Focus:** Scale and Innovation

**Key Deliverables:**
- Onboard new Forwood One clients (April/May)
- Launch Forwood One V3 (UAT and ready for CRM client migration)
- Conduct NPS (including Forwood One clients)
- Launch AI features from innovation stream

**Milestones:**
- ✅ New Forwood One clients onboarded
- ✅ V3 platform ready for CRM client migration
- ✅ NPS survey completed
- ✅ AI features launched

**Success Metrics:**
- Customer onboarding success rate
- NPS score targets met
- AI feature adoption
- CRM migration pathway validated

## Roadmap Themes

### Forwood One Platform Launch
- **Focus:** Launch Forwood One platform and achieve commercial traction with 5 new customer deals
- **Key Initiatives:** [Forwood One Platform Launch](Forwood-One-platform-launch.md)
- **Timeline:** Q1-Q4 FY26

### Customer Migration
- **Focus:** Migrate existing customers from Safety+, CRM+, and Risk+ platforms to Forwood One
- **Key Initiatives:** [Forwood One Platform Launch](Forwood-One-platform-launch.md)
- **Timeline:** Q2-Q4 FY26

### Product Excellence
- **Focus:** Deliver design system, achieve usability targets, and ensure positive customer experience
- **Key Initiatives:** [Forwood One Platform Launch](Forwood-One-platform-launch.md)
- **Timeline:** Q1-Q4 FY26

## Key Initiatives

- [Forwood One Platform Launch](Forwood-One-platform-launch.md)

## Product Outcomes

What product outcomes support our tactical roadmap?
- [Forwood One NPS Score](oculus-nps-score.md)
- [Usability Testing Success Rate](usability-testing-success-rate.md)
- [Forwood One Design System](04-opportunities/02-product-outcomes/oculus-design-system.md)
- [Forwood One Customer Experience](oculus-customer-experience.md)

## Tactical Priorities

### Must Have (Critical Path)
1. Forwood One platform launch and stability
2. Customer migration capability
3. Sales enablement and pipeline development
4. Design system and UX improvements

### Should Have (High Value)
1. Native app launch
2. AI feature development
3. Risk+ integration validation
4. NPS measurement and improvement

### Nice to Have (Enhancement)
1. Advanced analytics features
2. Additional integrations
3. Enhanced reporting capabilities

## Resource Considerations

### Team Capability
- Onboard Product Designer in Q2
- Embed agentic product processes in Q1
- Build team capability throughout the year

### Dependencies
- Sales pipeline development for Q2-Q3
- Customer readiness for migration in Q3
- Technical infrastructure for scale

## Risks & Mitigation

### Key Risks
- **Timeline pressure:** Bringing Forwood One to market within 12 months requires balancing rapid customer acquisition with complex migration
- **Quality vs. Speed:** Need to maintain delivery quality while accelerating
- **Customer readiness:** Migration depends on customer readiness and support

### Mitigation Strategies
- Accelerate discovery and delivery quality
- Empower team through collaboration, culture, upskilling and AI-enabled tools
- Embed culture of product discovery focused on deeply understanding industry problems
- Phased migration approach with strong customer support

## Notes

- Success depends on accelerating discovery and delivery quality while empowering the team through collaboration, culture, upskilling and AI-enabled tools
- Embedding a culture of product discovery focused on deeply understanding industry problems, not just requested solutions, will be key to success
- Balancing rapid customer acquisition with complex migration of existing clients requires careful planning and execution

## Industry Insights & Innovation

### Site-Specific Checklist Builder & Verification Scheduler

Guided by industry insight, we continue to develop advanced CRM+ methodology capabilities. The Site-Specific Checklist Builder & Verification Scheduler enables governed tailoring from a single master checklist, with a central library and smart scheduling that deliver full oversight in a fraction of the time.

**Case Study:** [Site-Specific Checklist Builder & Verification Scheduler Case Study](site-specific-checklist-builder-case-study.md)

**Impact:** What once took weeks of workshops and manual coordination can now be completed in a single day — with full oversight, speed, and assurance.

## Related Documents

- [Roadmapping Framework](roadmapping-framework.md) - 4-layer roadmap structure and methodology
- [Strategic Roadmap (2-Year Vision)](strategic-roadmap.md) - Long-term vision and strategy
- [Strategic Focus Areas](focus-areas.md)
- [Vision](vision-statements.md)
- [Business Outcomes](04-opportunities/01-business-outcomes/index.md)
- [Site-Specific Checklist Builder Case Study](site-specific-checklist-builder-case-study.md)
- [CRM Operational Pillars](crm-operational-pillars.md)
- [Inbox Item: 2-Year Strategy & Roadmap Overview](../../00-inbox/raw/2025-12-01-2-year-strategy-roadmap.md)
- [Inbox Item: Product – One Page Strategy (FY 25-26)](../../00-inbox/raw/2025-01-27-strategy-product-one-page-fy25-26.md)
- [Inbox Item: Forwood CRM Vision & Executive Roadmap (Speaker Notes for Ferrovial)](../../00-inbox/raw/2025-12-01-ferrovial-crm-vision-executive-roadmap-speaker-notes.md)

