# Opportunity: Hedno Stability and Reliability

**Status:** Archived  
**Created:** 2025-12-08  
**Last Updated:** 2026-01-03  
**Priority:** High

## Overview

Hedno customers have deep sensitivity to breakages in Safety+; unstable releases have caused their workforce to reject the tool, creating significant adoption challenges.

## Customer Problem

Hedno customers are experiencing:
- Deep sensitivity to breakages in Safety+ platform
- Unstable releases that have caused their workforce to reject the tool
- Concerns about reliability and stability that impact platform adoption
- Need for reassurance that release processes have been tightened

This problem is particularly acute because:
- Their original purchase driver was Geomap, not CRM
- They have an extremely hierarchical organisational culture requiring precise language and framing
- They heavily use DSS-built content rather than Forwood content, requiring careful mapping

## Evidence

What evidence supports this opportunity?
- [EDP and HEDNO Exec Alignment Meeting Research](../../../03-discovery/research/user-research/edp-hedno-exec-alignment-meeting-2025-12.md)
- Hedno customer feedback highlighting stability and reliability concerns
- Workforce rejection of the tool due to unstable releases
- Need for reassurance about release process improvements

## Impact

### Potential Impact on Outcomes
- **Product Outcome:** Link to product outcomes related to platform stability and reliability
- **Business Outcome:** [Forwood One Migration Success](../../01-business-outcomes/forwood-one-migration-success.md)

### Expected Impact
- Improved workforce adoption and acceptance of the platform
- Rebuilt trust through stable, reliable releases
- Better alignment with Hedno's hierarchical culture and precise requirements
- Support for successful migration to Forwood One

## Related Outcomes

- **Product Outcome:** [Platform Readiness Metrics](../../02-product-outcomes/platform-readiness-metrics.md)
- **Business Outcome:** [Forwood One Migration Success](../../01-business-outcomes/forwood-one-migration-success.md)

## Proposed Solutions

Link to solutions that address this opportunity:
- [Solutions will be added as they are identified](../../04-solutions/active/)

## Research and Discovery

Link to related research:
- [EDP and HEDNO Exec Alignment Meeting Research](../../../03-discovery/research/user-research/edp-hedno-exec-alignment-meeting-2025-12.md)

## Insights

Key insights that led to identifying this opportunity:
- Hedno has deep sensitivity to breakages due to past unstable releases
- Workforce rejection of the tool is a critical adoption challenge
- Release processes must emphasize stability and quality to rebuild trust
- Focus on stability, reliability, and ease of use (particularly for field users)

## Notes

- Hedno requires focus on stability, reliability, and ease of use
- Reassure that release processes have been tightened: regression testing, QA governance, and gated deployments
- Begin shifting mindset from bespoke requests to future configurability
- Acknowledge legacy issues in Safety+ without undermining trust
- Migration timeline: mid-2026 is the safest headline, but messaging needs flexibility

## Decision History

### Prioritization Decision - 2025-12-17

**Decision ID:** DEC-2025-12-17-002  
**Decision:** Keep in Backlog  
**Decision Maker:** Human  
**Status:** Superseded

**Rationale:** 
- Opportunity Score: 63 (Very High priority - Importance: 9/10, Satisfaction: 3/10)
- Strong evidence from research sources, customer feedback, and workforce rejection
- Critical customer (Hedno) with deep sensitivity to breakages and workforce rejection
- Decision to keep in backlog for now, likely due to resource constraints or timing considerations

**Supporting Evidence:**
- Links to "Forwood One Migration Success" business outcome
- Links to "Platform Readiness Metrics" product outcome
- Evidence from EDP and HEDNO Exec Alignment Meeting Research
- Customer feedback highlighting stability and reliability concerns

### Archive Decision - 2026-01-03

**Decision ID:** DEC-2026-01-03-006  
**Decision:** Archive  
**Decision Maker:** Human  
**Status:** Active

**Rationale:**
- This opportunity relates to Safety+ platform stability and reliability issues, not Forwood One platform
- Forwood One is the strategic platform direction, and Safety+ issues are legacy platform concerns
- Customer needs for stability and reliability are being addressed through Forwood One migration and platform readiness opportunities
- Archiving this opportunity as it is no longer relevant for Forwood One platform roadmap

**Supporting Evidence:**
- Opportunity focuses on Safety+ platform breakages (legacy platform)
- Strategic direction is Forwood One platform, not Safety+ improvements
- Related customer needs are addressed through "Customers Need Confidence in Migration Success" opportunity (Active)
- Hedno stability concerns are addressed through Forwood One migration success criteria

**Previous Decision:** DEC-2025-12-17-002

## Related Documents

- [Inbox Item: EDP and HEDNO Exec Alignment Meeting](../../../../00-inbox/raw/2025-12-08-edp-and-hedno-exec-alignment-meeting-[INBOX].md)

